...
import {
  ...
  installFailLoud,
  installUncaughtWatchCapacityGuard,
  loadOptionalPatches,
  loadOverlayPatches,
  loadProfile,
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  ...
  if (!signalShutdown.signal.aborted
    ...
    // chokidar 4 can escape an early fs.watch capacity failure (inotify
    // instance/watch budgets) as an uncaught exception before its own error
    // listener attaches; the guard degrades that case to "no hot reload"
    // instead of killing the surface. Removed once the watchers are up.
    const uninstallWatchGuard = installUncaughtWatchCapacityGuard(NAME)
    try {
