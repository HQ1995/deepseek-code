...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   Dscode agent server starting...");
    eprintln!();
...
async fn run_setup_command(json: bool) {
    ...
    if !managed_config::has_principal() {
        ...
        eprintln!();
        eprintln!(
            "If you don't have a deployment key, contact your organization's Dscode administrator."
        );
        std::process::exit(1);
    }
    if json {
...
fn main() {
    ...
    // machine never shares them. Precedence: GROK_HOME (upstream seam,
    // explicit always wins) > DSC_HOME > $DSH_HOME/dsc-tui > ~/.dsh/dsc-tui.
    ...
    if let Err(e) = xai_grok_config::validate_requirements() {
        eprintln!("Couldn't start Dscode: {e}");
        eprintln!();
        eprintln!(
            "Update Dscode to a version the policy allows, or ask your administrator \
    ...
    if xai_grok_shell::util::config::load_crash_handler_enabled_sync() {
        ...
        if let Some(report) = xai_crash_handler::check_previous_crash(&crash_dir) {
            eprintln!("Dscode crashed during your last session.");
