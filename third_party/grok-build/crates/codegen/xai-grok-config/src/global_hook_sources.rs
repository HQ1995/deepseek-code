...
pub enum GlobalHookSourceError {
    ...
    #[error("Dscode hooks directory has wrong type (expected real directory): {path}")]
    InvalidHooksDir { path: PathBuf },
    #[error("Dscode hooks-paths registry has wrong type (expected real file): {path}")]
    InvalidRegistryFile { path: PathBuf },
    #[error("cannot create Dscode hooks directory {path}: {source}")]
    CreateHooksDir {
        path: PathBuf,
        #[source]
        source: io::Error,
    },
    #[error("cannot create Dscode hooks-paths registry {path}: {source}")]
    CreateRegistryFile {
        path: PathBuf,
        #[source]
        source: io::Error,
