...
pub(crate) const ZDR_RESTRICTED_MESSAGE: &str = "Video generation tools are unavailable under zero data retention (ZDR). To re-enable, either supply a user-hosted storage bucket (see https://docs.x.ai/build/settings/zdr-video-storage) or turn off /privacy mode to disable ZDR for all Deepseek Code requests (including code). Restart Dscode after changing the config for it to take effect. Relay this message to the user verbatim; do not retry this tool.";
