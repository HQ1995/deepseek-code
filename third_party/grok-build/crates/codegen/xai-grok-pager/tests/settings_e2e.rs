...
const ALL_SETTINGS_EXERCISED: &[&str] = &[
    ...
    // DIVERGENCE(dscode): no "coding_data_sharing" row (coding-data sharing removed).
...
fn registry_kind_membership_through_pr_14() {
    ...
    assert_eq!(
        ...
        vec![
            "auto_dark_theme",
            "auto_light_theme",
            // DIVERGENCE(dscode): "coding_data_sharing" is not registered.
            "default_selected_permission",
            "follow_up_behavior",
            "hunk_tracker_mode",
            "keep_text_selection",
...
fn docs_footer_renders_for_browse_and_picker() {
    ...
    for fixture_label in ["browse", "picker"] {
        ...
        // DIVERGENCE(dscode): the footer names the product ("Ask Dscode").
        assert!(
            all_text.contains("Ask Dscode"),
            "[{fixture_label}] docs footer (`Ask Dscode`) must appear in the rendered modal:\n\
             {all_text}"
        );
        assert!(
...
// coding_data_sharing
//
// DIVERGENCE(dscode): the coding-data sharing row, its consent banner and
// `/privacy` are removed. The snapshot mapping below stays in
// `settings/registry.rs`; the row's keyboard, picker, search and mouse tests
// are omitted.
// ---------------------------------------------------------------------------

#[test]
fn coding_data_sharing_is_not_registered() {
    let reg = SettingsRegistry::defaults();
    assert!(reg.find("coding_data_sharing").is_none());
    assert!(
        reg.search("privacy").is_empty(),
        "no setting may answer a 'privacy' search once the row is removed"
    );
}
...
// DIVERGENCE(dscode): `/privacy` is removed with coding-data sharing; the
// command must not come back with an upstream sync.
#[test]
fn privacy_slash_command_is_not_registered() {
    use xai_grok_pager::slash::commands::builtin_commands;
    use xai_grok_pager::slash::registry::CommandRegistry;

    let reg = CommandRegistry::new(builtin_commands());
    assert!(
        reg.get("privacy").is_none(),
        "/privacy must not be registered"
    );
}
