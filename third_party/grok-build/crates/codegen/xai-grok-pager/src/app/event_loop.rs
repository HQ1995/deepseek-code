...
pub(crate) async fn run(
    ...
    // deepseek-build: in leader-client mode the connected leader owns
    // authentication (the harness provider layer). Never show the local
    // x.ai login flow; auth_state stays Done.
    let needs_interactive_login = !app.leader_mode && (connection.needs_login || force_login);
    ...
    // Reassemble legacy X10 reports mangled by UTF-8-converting relays.
...
/// processing to fix paste on terminals without bracketed paste, filter leaked
/// CSI fragments, and recombine relay-mangled legacy X10 mouse reports.
async fn drain_and_process(
    ...
    let mut handle_one = |routed: &RoutedInputEvent| -> bool {
        ...
        match ev {
            Event::FocusGained => {
                // ConPTY-backed relays can drop SGR mouse mode on refocus and
                // fall back to X10. Reassert capture unless the user disabled it.
...
pub(crate) fn session_flags_for_effects(
    ...
    effects::SessionFlags {
        ...
        persona_override: app.persona_override.clone(),
