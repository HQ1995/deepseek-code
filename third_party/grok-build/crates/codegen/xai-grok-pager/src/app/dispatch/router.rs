...
use super::ctx::{
    active_agent_session_id, get_active_agent, get_active_agent_mut, navigate_clearing_selection,
    open_url_or_show, sync_sleep_inhibitor, with_active_agent, with_scrollback,
};
use super::dashboard::{
...
pub(crate) fn dispatch(action: Action, app: &mut AppView) -> Vec<Effect> {
    ...
    let effects = match action {
        ...
        Action::LoadCommandOptions { command, query } => {
            super::command_options::open(app, &command, &query)
        }
        ...
        Action::SelectPersona(name) => {
            ...
            // valid preset id (grok-leader loadSession).
            let reload = {
            ...
            dispatch_load_session(app, session_id, Some(cwd), chat_kind)
        }
        Action::ToggleCatalog => {
            if let Some(agent) = get_active_agent_mut(app) {
                agent.toggle_catalog();
            }
            vec![]
        }
