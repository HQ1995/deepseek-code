...
fn reseed_agent_screen_mode(agent: &mut AgentView, mode: ScreenMode) {
    ...
    if !mode.is_minimal() {
        // Minimal stamps entry display modes directly; rebuild fullscreen's
        // cached heights and fold groups before reusing its viewport.
...
mod tests {
    ...
    fn reseed_round_trip_flips_every_mode_derived_gate() {
        ...
        agent.stash_prompt_draft(crate::app::agent_view::StashCause::Chord);
        agent.prompt.set_text("current draft");
        agent.prompt.set_cursor(3);
        agent.session.enqueue_prompt("queued follow-up".into());
        agent
            .scrollback
            .push_block(crate::scrollback::block::RenderBlock::system("selected"));
        agent
            .scrollback
            .set_selected(Some(agent.scrollback.len() - 1));
        let selected = agent.scrollback.selected();
        ...
        for &(mode, minimal) in &[
            ...
            assert_eq!(agent.prompt.text(), "current draft");
            assert_eq!(agent.prompt.cursor(), 3);
            assert_eq!(
                agent.prompt_stash.as_ref().unwrap().prompt.text,
                "parked draft"
            );
            assert_eq!(
                agent.session.pending_prompts.front().unwrap().text,
                "queued follow-up"
            );
            assert_eq!(agent.scrollback.selected(), selected);
            let child = agent
                .subagent_views
                .values()
                .next()
