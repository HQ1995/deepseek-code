...
pub enum Command {
    ...
    /// Check terminal, clipboard, color, and input support without starting Dscode
    ...
    Login {
        ...
        #[arg(long, hide = true)]
        legacy: bool,
        /// Use Dscode OAuth via auth.x.ai.
...
/// Targets a running leader process by PID (used by `dscode leader` / `dscode workspace`).
#[derive(Debug, clap::Args, Clone, Default)]
pub struct LeaderTargetArgs {
    /// Leader process ID from `dscode leader list`.
    #[arg(long)]
    pub pid: Option<u32>,
}
#[derive(Debug, clap::Args, Clone)]
    ...
    pub fn canonical_plugin_dirs(&self) -> Vec<PathBuf> {
        self.plugin_dirs
            ...
            .filter_map(|p| match dunce::canonicalize(p) {
                ...
                Ok(_) => {
                    eprintln!(
                        "dscode: --plugin-dir {}: not a directory; skipping",
                        p.display()
                    );
                    None
                }
                Err(e) => {
                    eprintln!("dscode: --plugin-dir {}: {e}; skipping", p.display());
                    None
                }
            })
            .collect()
...
pub enum AgentCmd {
    ...
    Stdio,
    /// Run the agent headlessly over the Dscode WebSocket relay
...
pub struct PagerArgs {
    ...
    /// Default for all `dscode -p` runs is to wait (up to `--background-wait-timeout`)
    ...
    pub no_leader: bool,
    /// Initial prompt for the interactive session, e.g. `dscode "fix the bug"` or `dscode --worktree=feat "create this feature"`.
    #[arg(
        value_name = "PROMPT",
        conflicts_with_all = &["single",
        "prompt_json",
...
impl PagerArgs {
    ...
    /// whitespace. This is the `dscode "<prompt>"` launch form; the headless
