...
/// A picked bundle persona overrides the built-in plan profile and is stamped
/// as a plain string agentProfile for the harness leader to resolve.
#[serial_test::serial(GROK_AGENT)]
#[test]
fn persona_override_stamps_agent_profile_name() {
    let _env = without_grok_agent();
    let flags = SessionFlags {
        plan_mode: true,
        subagents: true,
        ask_user: false,
        persona_override: Some("minimal".to_string()),
        ..Default::default()
    };
    let meta = flags.to_meta().unwrap();
