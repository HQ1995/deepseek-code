...
pub(crate) fn execute(
    ...
    match effect {
        ...
        Effect::QueueSteer { session_id, id, expected_version } => {
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    let params = serde_json::json!({
                    ...
                    let notification = acp::ExtNotification::new(
                        "x.ai/queue/steer",
                        serde_json::value::to_raw_value(&params)
                            .expect("serialize queue/steer params")
                            .into(),
                    );
                    if let Err(e) = acp_send(notification, &tx).await {
                        tracing::warn!("Failed to send queue/steer notification: {e}");
                    }
                    TaskResult::CancelComplete
                });
        }
...
fn format_auth_lines(is_api_key_auth: bool, api_key_env_set: bool) -> String {
    if is_api_key_auth {
        ...
        return format!(
            "{method}  Run `dscode login` to use your SuperGrok subscription instead.\n"
        );
    }
