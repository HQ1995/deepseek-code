...
pub(crate) struct SessionFlags {
    ...
    /// Persona name picked from the leader bundle (x.ai/bundle/status
    /// personas). When set it wins over the built-in Build profiles and is
    /// stamped into _meta.agentProfile as a string so the harness leader
    /// resolves it to a dsh agent preset.
