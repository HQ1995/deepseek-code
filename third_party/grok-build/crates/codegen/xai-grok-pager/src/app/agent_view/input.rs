...
impl AgentView {
    ...
    /// [`Self::try_handle_esc_policy`] (running or cancelling → swallow plus
    /// the Ctrl+C hint), not detach to the dashboard. Detach mid-turn stays on
    /// Ctrl+\ / Left.
    ...
    fn handle_input_inner(
        ...
        if let Event::Key(key) = ev
            && key.kind != KeyEventKind::Release
            && registry.matches_id(ActionId::ToggleCatalog, key)
        {
...
mod mid_turn_esc_hint_tests {
    ...
    /// Mid-turn Esc never cancels: it names the registry cancel key instead, in
    /// every mode and from either pane, and the draft survives.
    #[test]
    fn mid_turn_esc_shows_ctrl_c_hint_instead_of_cancelling() {
        for (vim_mode, minimal, pane) in [
            ...
            if minimal {
                ...
                let _ = press_esc(&mut agent);
                assert_eq!(2, count_hint_lines(&agent), "a new user turn allows one more");
            } else {
                assert_eq!(
    ...
    /// While a cancel is already in flight, Esc is swallowed without a hint
    /// (Ctrl+C escalates to Quit there, so the hint would mislead) and never
    /// re-sends the cancel.
    #[test]
