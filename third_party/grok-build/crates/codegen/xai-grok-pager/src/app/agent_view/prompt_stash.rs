...
mod tests {
    ...
    #[test]
    fn steer_after_paste_keeps_steer_semantics_and_restores_the_stash() {
        use super::super::AgentDeferredSend;
        ...
        agent.paste_probe_in_flight = 1;
        let outcome =
            agent.handle_prompt_key_for_test(&KeyEvent::new(KeyCode::Enter, KeyModifiers::ALT));
        assert!(matches!(outcome, InputOutcome::Changed));
        assert_eq!(agent.deferred_send, Some(AgentDeferredSend::Steer));
        agent.paste_probe_in_flight = 0;
        let kind = agent.take_deferred_send_after_paste().unwrap();
        assert!(
            matches!(agent.resume_deferred_send(kind), Some(Action::Interject { text, .. }) if text == "one more thing")
        );
        assert!(agent.take_draft_consumed());
        agent.auto_restore_stash_after_send();
        assert_eq!(agent.prompt.text(), "stashed thought");
        assert!(agent.prompt_stash.is_none());
    }
