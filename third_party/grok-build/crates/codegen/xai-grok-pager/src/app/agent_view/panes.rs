...
impl AgentView {
    ...
    /// Kept next to handle_catalog_key so the toggle semantics stay in one
    /// place whether the trigger is a key or a slash command.
    pub(crate) fn toggle_catalog(&mut self) {
    ...
    pub(super) fn handle_catalog_key(
        ...
        use crate::views::overlay::{handle_overlay_key, handle_overlay_nav_key};
        if registry.matches_id(ActionId::ToggleCatalog, key) {
            self.catalog.overlay.toggle();
            self.catalog.on_state_change();
            if !self.catalog.overlay.focused {
                return InputOutcome::Action(Action::FocusScrollback);
            ...
            && key.modifiers == crossterm::event::KeyModifiers::NONE
        {
            if let Some((kind, name)) = self.catalog.selected_entry() {
                if kind == "persona" {
                    return InputOutcome::Action(Action::SelectPersona(name.to_owned()));
                }
