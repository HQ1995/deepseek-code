...
#[test]
fn viewer_quotes_released_unicode_drag() {
    use crossterm::event::{MouseButton, MouseEventKind};
    let mut agent = make_agent();
    let mut viewer = crate::views::block_viewer::BlockViewerPane::for_plain_text("t", "汉字 café");
    viewer.prepare_for_test(Rect::new(0, 0, 80, 24));
    viewer.handle_mouse(MouseEventKind::Down(MouseButton::Left), 2, 2);
    viewer.handle_mouse(MouseEventKind::Drag(MouseButton::Left), 8, 2);
    viewer.handle_mouse(MouseEventKind::Up(MouseButton::Left), 8, 2);
    assert!(viewer.text_drag.is_some_and(|drag| !drag.active));
    assert_eq!(viewer.selected_plain_text(), "字 café");
    agent.block_viewer = Some(viewer);
    agent.handle_block_viewer_key(&enter_key());
    assert_eq!(agent.prompt.text(), "> 字 café\n\n");
}

#[test]
fn viewer_filtered_drag_quotes_only_visible_rows() {
    use crate::views::list_pane::FilterMatcher;
    use crossterm::event::{MouseButton, MouseEventKind};

    let mut viewer = crate::views::block_viewer::BlockViewerPane::for_plain_text(
        "t",
        "hidden first\nkeep 汉字\nhidden middle\nkeep café\nhidden last",
    );
    viewer
        .list_state
        .set_filter(Some(FilterMatcher::substring("keep")));
    viewer.prepare_for_test(Rect::new(0, 0, 80, 24));
    viewer.handle_mouse(MouseEventKind::Down(MouseButton::Left), 0, 0);
    viewer.handle_mouse(MouseEventKind::Drag(MouseButton::Left), 8, 1);
    viewer.handle_mouse(MouseEventKind::Up(MouseButton::Left), 8, 1);
    assert_eq!(viewer.selected_plain_text(), "keep 汉字\nkeep café");
    let mut buf = ratatui::buffer::Buffer::empty(Rect::new(0, 0, 80, 24));
    viewer.render_text_drag_overlay(&mut buf);
    assert_ne!(buf[(0, 0)].style(), buf[(0, 2)].style());
    assert_ne!(buf[(0, 1)].style(), buf[(0, 2)].style());
}

#[test]
fn viewer_resume_does_not_cross_unanchored_tasks() {
    let mut agent = make_agent();
    agent.session.bg_tasks.insert(
        "first".into(),
        test_bg_task("first", "one\ntwo\nthree", None),
    );
    agent.session.bg_tasks.insert(
        "second".into(),
        test_bg_task("second", "alpha\nbeta\ngamma", None),
    );
    assert!(agent.show_bg_task_viewer("first"));
    let viewer = agent.block_viewer.as_mut().unwrap();
    viewer.prepare_for_test(Rect::new(0, 0, 80, 2));
    viewer.select_body_line_for_test(2);
    agent.dismiss_block_viewer();
    assert!(agent.show_bg_task_viewer("second"));
    let viewer = agent.block_viewer.as_mut().unwrap();
    viewer.prepare_for_test(Rect::new(0, 0, 80, 2));
