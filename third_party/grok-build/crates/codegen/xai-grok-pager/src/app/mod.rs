...
pub async fn run(
    ...
    // dscode has no embedded fallback; a leader failure is terminal.
...
fn print_exit_resume_hint(info: &ExitInfo, max_width: usize, w: &mut impl Write) {
    ...
    if info.minimal {
        let _ = writeln!(w, "  dscode --minimal --resume {}", info.session_id);
    } else {
        let _ = writeln!(w, "  dscode --resume {}", info.session_id);
    }
}
...
mod tests {
    ...
    #[test]
    fn cli_command_name_is_dscode() {
        use clap::CommandFactory;
        assert_eq!(PagerArgs::command().get_name(), "dscode");
    }
    #[test]
    fn cli_help_output_header() {
        ...
        assert_eq!(
            ...
            vec![
                "Deepseek Code TUI",
                "",
                "Usage: dscode [OPTIONS] [PROMPT] [COMMAND]",
                "",
                "Arguments:",
            ]
        );
    ...
    fn print_exit_resume_hint_includes_minimal_flag() {
        ...
        assert_eq!(
            String::from_utf8(buf).unwrap(),
            "\nResume this session with:\n  dscode --minimal --resume sess-abc\n"
        );
    }
    #[test]
    ...
    fn print_exit_resume_hint_truncates_summary_to_width() {
        ...
        assert!(out.contains(&format!("\n> {}…\n", "p".repeat(17))));
        assert!(out.contains(&format!("\n  {}…\n", "r".repeat(17))));
        assert!(out.contains("  dscode --resume sess-abc\n"));
    }
    #[test]
