...
impl TitleManager {
    ...
    pub fn reset(&mut self) -> String {
        let esc = build_title_escape("dscode");
        self.last_title.clear();
        self.last_title.push_str("dscode");
...
mod tests {
    ...
    fn session_name_and_grok_joined_with_separator() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "my project - dscode");
    }
    ...
    fn spinner_only_shown_when_active() {
        ...
        mgr.update(&state);
        assert!(mgr.last_title.contains(" - dscode"));
    ...
    fn activity_shows_waiting_when_busy_without_activity() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Waiting - dscode");
    }
    ...
    fn activity_prefers_specific_activity_over_waiting() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Thinking - dscode");
    }
    ...
    fn model_item_shown_when_present() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "grok-3 - dscode");
    }
    ...
    fn turn_timer_shown_when_above_one_second() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "42s - dscode");
    }
    ...
    fn reset_clears_state_and_emits_grok() {
        ...
        mgr.update(&state);
        assert_ne!(mgr.last_title, "dscode");
    ...
    fn default_config_active_turn_with_permissions() {
        ...
        for t in [&t1, &t2] {
            assert!(t.contains("dscode"), "title missing 'dscode': {t}");
    ...
    fn all_items_present_in_order() {
        ...
        assert_eq!(
            mgr.last_title,
            "Thinking - proj - grok-3 - workspace - dscode"
        );
    }
