    ...
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::UnknownId(id) => write!(
                formatter,
                "`{id}` is not an available Doctor fix. Run `dscode doctor fix` to list available fixes."
            ),
            Self::PlatformUnsupported => write!(
                formatter,
                "Automatic SSH setup is not available on Windows. Run `{SSH_WRAP_ONE_OFF}` when needed."
            ),
            Self::HomeUnavailable => formatter.write_str("Dscode could not find your home directory."),
            ...
            Self::ByobuConfigUnavailable => formatter.write_str(
                "Dscode could not determine Byobu's effective config directory. Keep `BYOBU_CONFIG_DIR` set in this session, then run the fix again.",
            ),
            Self::UnsafeDirectory { label, path } => write!(
                formatter,
                "Dscode refused unsafe {label} `{}`. Use a non-root absolute directory without control characters, `~`, `.` or `..` components.",
                path.display()
            ),
                    ...
                    || detail.contains("`ssh` fish function") =>
            {
                write!(
                    formatter,
                    "Dscode found an existing SSH alias or function in {} and did not change it: {detail}",
                    path.display()
                )
            }
            Self::ExistingCustomization { path, detail } => write!(
                formatter,
                "Dscode found an existing customization in {} and did not change it: {detail}",
                path.display()
            ),
            ...
            Self::PostconditionFailed => formatter
                .write_str("The configuration changed, but Dscode could not verify the SSH alias."),
            Self::TmuxPostconditionFailed => formatter.write_str(
                "The configuration changed, but Dscode could not verify the managed tmux option.",
            ),
        }
    }
}
...
const FIX_REGISTRY: &[FixSpec] = &[
    ...
    FixSpec {
        ...
        command: "dscode doctor fix terminal.dcs-passthrough",
    ...
    FixSpec {
        ...
        command: "dscode doctor fix terminal.tmux-extended-keys",
    ...
    FixSpec {
        ...
        command: "dscode doctor fix terminal.tmux-truecolor",
...
pub(crate) fn human_fix_command(id: DiagnosticId) -> Option<String> {
    fix_spec(id).map(|spec| format!("dscode doctor fix {}", spec.handle))
}
    ...
    for (id, handle, availability) in fixes {
        ...
        match availability {
            AutomaticFixAvailability::Here => output.push_str(&format!(
                "    Run: dscode doctor fix {handle}\n    In Dscode: /doctor fix {handle}\n"
            )),
            AutomaticFixAvailability::RunLocally => output.push_str(&format!(
                "    On your local computer, run: dscode doctor fix {handle}\n"
            )),
        }
    }
    output
...
pub(crate) fn format_fix_preview(plan: &FixPlan) -> String {
    ...
    match &change.backup_path_hint {
        Some(path) => {
            let _ = writeln!(
                output,
                "\nBackup will be saved to: {}\nIf that file exists, Dscode will choose a unique name.",
                preview_path(path)
            );
        }
    ...
    match &plan.payload {
        FixPayload::SshWrap(_) => {
            output.push_str(
                "\nWhat this changes:\n  In new interactive shells, `ssh ...` runs as `dscode wrap ssh ...`.\n",
            );
            let _ = writeln!(
                output,
...
fn plan_ssh_wrap(
    ...
    Ok(FixPlan {
        ...
        caveats: vec![
            ...
            "`dscode wrap` starts the SSH process directly, so the alias does not loop.",
            "Dscode checks this file for direct SSH aliases and functions. Review sourced files, plugins, and generated shell setup yourself.",
        ],
