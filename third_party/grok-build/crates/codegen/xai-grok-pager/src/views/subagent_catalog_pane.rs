//! Subagent catalog pane — browseable list of bundled presets/roles/agents.
//!
//! Read-only pane that renders grouped entries from [`BundleState`]. Headers
//! (Presets, Roles, Agents) are non-selectable; items below each header
...
struct CatalogEntry {
    ...
    /// Selection value returned by selected_entry. For personas this is the
    /// preset id (label is the localized display name); other kinds fall back
    /// to the label.
    value: Option<String>,
}
...
impl SubagentCatalogPane {
    ...
    pub fn sync_from_bundle(&mut self, state: &BundleState) {
        ...
        let groups: [(&str, &'static str, &[String]); 3] = [
            ("Presets", "persona", &state.personas),
        ...
        for (name, kind, items) in &groups {
            ...
            for (idx, item) in items.iter().enumerate() {
                ...
                // Presets show the leader's localized display name but select
                // the preset id; roles/agents keep the id as both.
                let detail = if *kind == "persona" {
                    state.persona_details.get(idx)
                } else {
                    None
                };
                let display = detail
                ...
                let spans = if let Some(d) = desc {
                    vec![
                        Span::styled(format!("  {display}"), item_style),
                        Span::styled(format!(" \u{2014} {d}"), desc_style),
                    ]
                } else {
                    vec![Span::styled(format!("  {display}"), item_style)]
                };
                self.entries.push(CatalogEntry {
                    id: hasher.finish(),
                    label: display,
    ...
    pub fn selected_entry(&self) -> Option<(&str, &str)> {
        ...
        Some((entry.kind?, entry.value.as_deref().unwrap_or(&entry.label)))
    }
...
mod tests {
    ...
    fn sync_with_data_produces_grouped_entries() {
        ...
        assert_eq!(pane.entries[0].label, "Presets");
    ...
    fn selected_entry_returns_kind_and_name() {
        ...
        // [0]=Presets, [1]=researcher, [2]=Roles, [3]=reviewer, [4]=Agents, [5]=default
    ...
    fn selected_entry_returns_none_for_header() {
        ...
        // Select the "Presets" header (entries[0])
