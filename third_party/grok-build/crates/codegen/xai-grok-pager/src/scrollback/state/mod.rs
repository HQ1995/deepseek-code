...
impl ScrollbackState {
    ...
    /// completion):
    ///
    /// - Same-kind swaps keep the entry's current mode, so a manual expand
    ///   survives progress updates, refinements, and completion.
    ...
    pub fn finish_running_with_time(&mut self, id: EntryId, thinking_time_ms: Option<i64>) {
        ...
        if let Some(entry) = self.get_by_id_mut(id) {
            ...
            if respect_manual_folds && entry.display_mode_pinned {
                ...
                // Collapsed stays folded; finish must not snap it open.
