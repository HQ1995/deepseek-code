...
const MSG_RUN_UPDATE_MANUAL: &str = "Run `dscode update` to get the latest version.";
...
pub fn print_update_status(status: &UpdateStatus, json: bool) -> anyhow::Result<()> {
    ...
    if status.update_available {
        if let Some(latest_version) = status.latest_version.as_deref() {
            ...
            println!("A new version of Deepseek Code is available.");
        }
        return Ok(());
    }
...
async fn run_update_subcommand(
    ...
    match run_mode {
        UpdateRunMode::Blocking => {
            ...
            if !status.success() {
                anyhow::bail!("dscode update failed with {}", status);
            }
            Ok(None)
        }
...
async fn download_verified_from_base(
    ...
    eprintln!("  Downloading dscode v{} ({})...", version, platform);
    ...
    rename_result.map_err(|e| {
        anyhow::anyhow!(
            "cannot rename locked executable {}: {e}\n\
             Close all running dscode sessions and retry.",
            dest.display(),
        )
    })?;
    match tokio::fs::copy(src, dest).await {
...
fn warn_if_other_grok_processes_running() {
    ...
    if let Ok(output) = cmd.output() {
        ...
        if !other_pids.is_empty() {
            eprintln!(
                "  ⚠ Warning: {} other dscode process(es) detected.",
                other_pids.len()
            );
            ...
            eprintln!("    Consider closing other dscode sessions before updating.");
            eprintln!();
        }
    }
}
...
pub async fn run_update(
    ...
    if let Some(version) = pinned_version {
        ...
        eprintln!("  ✓ dscode v{} installed successfully!", version);
        eprintln!("  Please restart Dscode.");
    ...
    {
        eprintln!(
            "Forcing reinstall of Dscode {} (already up to date)",
            effective_current
        );
        &effective_current
    } else {
        eprintln!("Updating Dscode {} → {}", effective_current, install_target);
        &install_target
    };
...
async fn refresh_deployment_config() {
    ...
    match xai_grok_shell::managed_config::sync().await {
        ...
        Err(e) if e.is_retryable() => {
            ...
            eprintln!("  Couldn't apply managed configuration. Run `dscode setup` to retry.");
        }
