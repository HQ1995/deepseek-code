...
it.each([0, 1, 2].flatMap(version => ['dscode/model-selected', 'model/selected'].map(type => ({ version, type }))))('migrates $type from V$version through native V4 publication and preserves the original', async ({ type, version }) => {
  ...
  try {
    ...
    expect(await readFile(join(directory, 'session.v4.jsonl'), 'utf8')).toContain('model/selection')
    if (process.env.DSCODE_E2E_PREVIOUS_RUNTIME) {
      // A real older runtime must refuse V4, never resume the stale V0/V1/V2 file.
      const result = await promisify(execFile)(process.execPath, ['--input-type=module', '-e', `
        ...
        try {
          ...
          console.log('older runtime refused V4');
        } finally { await ctx.fiber.dispose(); }
      `, root], { timeout: 15000 })
      expect(result.stdout).toContain('older runtime refused V4')
      expect(await readFile(path, 'utf8')).toBe(bytes)
    }
    uninstall()
...
it('does not weaken native validation or publish a generation for malformed legacy events', async () => {
  ...
  try {
    ...
    expect(await readFile(path, 'utf8')).toBe(bytes)
    expect(await readdir(directory)).not.toContain('session.v4.jsonl')
  } finally {
    await ctx.fiber.dispose()
    await rm(root, { recursive: true, force: true })
  }
...
it.each([false, true])('reopens a V3 tool conversation as V4 and replays its result (isError=%s)', async isError => {
  const root = await mkdtemp(join(tmpdir(), 'dscode-v3-tool-')), ctx = new Context()
  try {
    const id = SessionId('v3-tool'), directory = join(root, '_no-cwd', id)
    await mkdir(directory, { recursive: true })
    ...
    const rows = [
      ...
      { type: 'assistant/message', surfaceOp: 'append', data: { turn: 1, step: 1, stream: [], message: {
        id: 'assistant-one', role: 'assistant', source: { kind: 'model', provider: 'mock', model: 'mock' },
        content: [{ type: 'tool-call', id: 'read-one', name: 'read', arguments: '{"path":"/fixture.txt"}' }],
      } } },
      { type: 'tool/call', data: { turn: 1, step: 1, callId: 'read-one', name: 'read', arguments: '{"path":"/fixture.txt"}' } },
      { type: 'tool/result', surfaceOp: 'append', data: { turn: 1, step: 1, message: { id: 'result-one', role: 'user', source: { kind: 'tool', callId: 'read-one' },
        content: [{ type: 'tool-result', toolCallId: 'read-one', isError, content: [{ type: 'text', text: 'saved result' }] }] } } },
      { type: 'step/end', data: { turn: 1, step: 1 } },
      { type: 'turn/end', data: { turn: 1, reason: { kind: 'completed' } } },
    ]
    const bytes = [{ type: 'session', version: 3, id, createdAt: 1, isSeeded: false, delegationDepth: 0 },
      ...rows.map((row, seq) => ({ ...row, seq, time: seq + 1 }))].map(row => JSON.stringify(row)).join('\n') + '\n'
    await writeFile(path, bytes)
    await ctx.plugin(JsonlSessionPersistence, { root, compression: 'none' })
    installLegacySessionMigration(ctx.sessionPersistence)
    for (const mode of ['write', 'read'] as const) {
      ...
      try {
        ...
        expect(result.data).toMatchObject({ message: { role: 'tool', toolCallId: 'read-one', isError, content: [{ type: 'text', text: 'saved result' }] } })
        expect(sessionEventToUpdates(result, { replay: true })).toContainEqual(expect.objectContaining({ sessionUpdate: 'tool_call_update', toolCallId: 'read-one', status: isError ? 'failed' : 'completed',
          content: [{ type: 'content', content: { type: 'text', text: 'saved result' } }] }))
      } finally { await handle.close() }
    }
    expect(await readFile(path, 'utf8')).toBe(bytes)
    expect(await readdir(directory)).toContain('session.v4.jsonl')
