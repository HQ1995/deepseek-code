...
describe('shipped composition', () => {
  ...
  it('mounts native session services before the preset catalog', () => {
    ...
    expect(analysis.insertedRows).toEqual(['session-reference', 'schedule', 'terminals', 'terminal-bash', 'subagent-model-selection-settings', 'agent-preset-registry', 'cordis-host-runner', 'grok-leader', 'agent-team', 'dscode-browser'])
  })
})
