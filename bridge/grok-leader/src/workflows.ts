...
/** Fold each durable workflow event once per attached session. */
export class WorkflowIndex {
  private readonly runs = new Map<string, { name: string; started: number; time: number; status?: string; members: Map<number, Member> }>()
  ...
  updates(live: ReadonlyMap<string, LiveWorkflow>, now: number, runId?: string) {
    const selected = runId === undefined ? undefined : this.runs.get(runId)
    const runs = runId === undefined ? [...this.runs] : selected === undefined ? [] : [[runId, selected] as const]
