...
Upstream has no version negotiation step: `LEADER_PROTOCOL_VERSION` is `1` (`protocol.rs:125`), registration accepts any advertised version, and only the control surface rejects a mismatch. The dscode fork deliberately tightens this seam in `client.rs`: a missing or different advertised protocol version fails registration before ACP traffic starts. When the registered client's binary version differs from the leader's, an upstream leader pushes a one-shot ACP notification `x.ai/leader/version_mismatch` (`server.rs:1474-1504`, emitted at registration `server.rs:1669`).
