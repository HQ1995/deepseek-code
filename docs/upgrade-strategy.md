...
| Model-request retries and typed turn failures | A scheduled retry shows the TUI's `Retrying (attempt N)…` status; a failed turn shows its typed banner (status headline, context overflow, timeout, empty response, connection) live and on resume, and the prompt error names status, code and provider request id. Missing or unusable keys still settle as the `/provider` fix |
| Automatic compaction | The TUI's "Context N% full. Compacting…" line and spinner at the start, then "Context compacted: before → after tokens (Ns)" or the failure, live and on resume; manual `/compact` keeps its command flow |
| Plan mode and plan review | `exit_plan_mode` opens the TUI's full plan approval view: approve, request changes with feedback, or abandon (plan mode off); the plan-mode indicator follows DSH's committed mode after `/plan`, Shift+Tab, a review, resume and fork |
| Turns started by a reminder, job, teammate, subagent or goal | A system line heads the turn with DSH's own trigger title and the producer's summary (for example `Background task updated: …`, `Team message received from …`), live and on resume |
