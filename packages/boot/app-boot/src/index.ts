...
/**
 * Whether a failure means the system ran out of file-watch capacity (the
 * per-user inotify instance or watch budgets), not a configuration error:
 * fs.watch reports EMFILE when the inotify-instance budget is exhausted and
 * ENOSPC when the watch budget is exhausted. A live-reload watcher must
 * degrade to "no hot reload this run" instead of failing the boot or the
 * whole process.
 * @param error - the failure to classify.
 * @returns true for a watch-capacity failure, false otherwise.
 */
export function isWatchCapacityError(error: unknown): boolean {
  const record = error as { code?: unknown; syscall?: unknown } | null | undefined
  if (record === null || record === undefined) return false
  return (record.code === 'EMFILE' || record.code === 'ENOSPC') && record.syscall === 'watch'
}
...
export async function watchUserPatches(
  ...
  try {
    ...
    // A saturated file-watch budget must turn live reload off for this run,
    // not fail the boot (or the surface) over a convenience watcher.
    if (isWatchCapacityError(error)) {
      ctx.logger.warn(binName + ': live user-patch reload is unavailable this run (file-watch capacity exhausted)')
      return async () => {}
    }
    throw error
  }
...
/**
 * Install a scoped uncaughtException guard for the user-patch watcher setup
 * window. chokidar 4 emits an early fs.watch failure (EMFILE/ENOSPC on a
 * saturated inotify budget) before its own error listener is attached, which
 * escapes as an uncaught exception and kills the surface. A watch-capacity
 * failure is swallowed with one warning (hot reload stays off this run);
 * every other exception rethrows, preserving the default fatal behavior.
 * @param binName - the diagnostic prefix on the warning line.
 * @param proc - the process slice to register on; tests inject a fake.
 * @returns the uninstaller that removes the handler.
 */
export function installUncaughtWatchCapacityGuard(
  binName: string,
  proc: FailLoudProcess = process,
): () => void {
  const handler = (error: unknown): void => {
    if (!isWatchCapacityError(error)) throw error
    proc.stderr.write(binName + ': warning: live user-patch reload is unavailable this run (file-watch capacity exhausted)\n')
  }
...
export function installFailLoud(
  ...
  const handler = (err: unknown): void => {
    ...
    // chokidar's initial scan escapes a saturated watch budget as an
    // unhandled rejection; the surface must survive with hot reload off.
