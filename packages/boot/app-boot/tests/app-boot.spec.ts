...
import {
  ...
  installFailLoud, installUncaughtWatchCapacityGuard, isWatchCapacityError,
  loadEnv, loadLayeredEnv, loadOverlayPatches, resolveConfigPath, type FailLoudProcess,
} from '../src/index.ts'
...
describe('installFailLoud', () => {
  ...
  it('swallows a file-watch capacity rejection with a warning and stays fatal for the next real failure', () => {
    const proc = fakeProc()
    installFailLoud(NAME, proc)
    const watchError = Object.assign(new Error('EMFILE: too many open files, watch'), { code: 'EMFILE', syscall: 'watch' })
    proc.handlers[0]!(watchError)
    expect(proc.exits).toEqual([])
    expect(proc.written[0]).toContain('file-watch capacity exhausted')
    proc.handlers[0]!(new Error('boom'))
    expect(proc.exits).toEqual([1])
    expect(proc.written[1]).toContain('boom')
  })
...
describe('isWatchCapacityError', () => {
  it('classifies only file-watch capacity failures', () => {
    expect(isWatchCapacityError(Object.assign(new Error('x'), { code: 'EMFILE', syscall: 'watch' }))).toBe(true)
    expect(isWatchCapacityError(Object.assign(new Error('x'), { code: 'ENOSPC', syscall: 'watch' }))).toBe(true)
    expect(isWatchCapacityError(Object.assign(new Error('x'), { code: 'EMFILE', syscall: 'open' }))).toBe(false)
    expect(isWatchCapacityError(new Error('EMFILE'))).toBe(false)
    expect(isWatchCapacityError('EMFILE')).toBe(false)
    expect(isWatchCapacityError(undefined)).toBe(false)
  })
})

describe('installUncaughtWatchCapacityGuard', () => {
  it('swallows a watch-capacity uncaught exception, warns, and rethrows everything else', () => {
    const proc = fakeProc()
    const uninstall = installUncaughtWatchCapacityGuard(NAME, proc)
    const watchError = Object.assign(new Error('EMFILE: too many open files, watch'), { code: 'EMFILE', syscall: 'watch' })
    expect(() => { proc.handlers[0]!(watchError) }).not.toThrow()
    expect(proc.exits).toEqual([])
    expect(proc.written[0]).toContain('file-watch capacity exhausted')
    const other = new Error('other')
    expect(() => { proc.handlers[0]!(other) }).toThrow(other)
    uninstall()
    expect(proc.handlers).toHaveLength(0)
  })
})
