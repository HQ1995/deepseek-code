...
describe('boot with user patches', () => {
  ...
  it('degrades to a no-op disposer with a warning when watcher startup hits file-watch capacity', async () => {
    const dir = tmp()
    const userDir = tmp()
    const filename = join(userDir, PROFILE_PATCH_FILENAME)
    const ctx = await boot(NAME, writeTree(dir))
    ...
    try {
      ...
      ctx.provide('hmr', { registerConfig: () => Promise.reject(watchError) })
  ...
  it('rethrows a non-capacity watcher startup failure', async () => {
    const dir = tmp()
    const userDir = tmp()
    const filename = join(userDir, PROFILE_PATCH_FILENAME)
    const ctx = await boot(NAME, writeTree(dir))
    try {
      ctx.provide('hmr', { registerConfig: () => Promise.reject(new Error('other failure')) })
      await expect(watchUserPatches(ctx, { binName: NAME, filename })).rejects.toThrow('other failure')
    } finally {
      await ctx.fiber.dispose()
    }
  })
